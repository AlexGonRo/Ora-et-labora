...
# Let's initialite some variables first...
$total_maintenance = array();   # Total number of resources that we need to keep all our buildings repaired
$buildings = array();   # Info about already built buildings
$possible_buildings = array(); # Info about buildings that aren't built yet

# Let's get a list of all possible buildings we could have.
$available_buildings_querry = mysqli_prepare($db,
    'SELECT id, name, img '
        . 'FROM buildings_info '
        . 'WHERE role= ? OR role=2');       // TODO Hardcoded value here, beware!
mysqli_stmt_bind_param($available_buildings_querry, "i", $role);
mysqli_stmt_bind_result($available_buildings_querry, $building_id, $building_name, $building_img);
mysqli_stmt_execute($available_buildings_querry);
mysqli_stmt_store_result($available_buildings_querry); # Mandatory to use this buffer, we have SQL sentences inside the loop

while (mysqli_stmt_fetch($available_buildings_querry)) {
    
    # Does the user have this building already?
    $building_info_query = mysqli_prepare($db,
    'SELECT a.id, a.preservation, a.level, b.time_left '
        . 'FROM buildings AS a LEFT JOIN buildings_current_lvlups AS b ON a.id=b.user_building_id '
            . 'WHERE a.owner_id= ? AND a.building_id = ?');
    mysqli_stmt_bind_param($building_info_query, "ii", $_SESSION['user_id'], $building_id);
    mysqli_stmt_bind_result($building_info_query, $user_building_id, $preservation, $level, $time_left);
    mysqli_stmt_execute($building_info_query);
    mysqli_stmt_store_result($building_info_query);
    
    
    if(mysqli_stmt_num_rows($building_info_query)>0){ # The user already has this building
        # Get the data
        mysqli_stmt_fetch($building_info_query);
        ...
        $max_level = false;
        $lvlup_array = get_building_lvlup_resources($building_id, $level+1);
        ...
        foreach($lvlup_array as $item_id => $item_quantity){
    ...
    } else { # This building could be built
        ...
        // Let's re-arrange all the information related to the lvlup resources
        $my_lvlup_array = array();
        foreach($lvlup_array as $item_id => $item_quantity){
            $item_name = $lvlup_names[$item_id];
            $item_icon = $lvlup_icons[$item_id];
...
mysqli_stmt_close($available_buildings_querry);
