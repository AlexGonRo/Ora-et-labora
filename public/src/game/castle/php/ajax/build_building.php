...
$required_resources = get_building_lvlup_resources($building_id, 1);
...
$get_lvlup_time_query = mysqli_prepare($GLOBALS['db'],
    "SELECT lvlup_time FROM buildings_lvlup_time WHERE building_id=? AND level=1");
mysqli_stmt_bind_param($get_lvlup_time_query, "ii", $building_id);
